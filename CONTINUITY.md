...
- Проактивный таймер P1_ — УБРАН (мусорные песни в аккаунте)
- Store: in-memory (рестарт сервера теряет сессии)
- Генерация: 5 вопросов → AI текст (Claude Sonnet 4.6) → редактирование → SUNO custom_generate
...
## State
- **Сейчас**: бот работает ✅, Claude Sonnet 4.6 генерирует тексты
- **Credits**: ~2200/2500 (использовано ~300 за тест)
- **AI модель**: anthropic/claude-sonnet-4.6 via OpenRouter
- **Деплой**: commit 3b997d2 на сервере
- **Cookie**: свежая (авто-обновлена 13.04)
- **P1_**: не критичен (cookie важнее)
...
- Если cookie устареет: авто-refresh сработает при следующей генерации
- Ручное восстановление cookie если авто не справится: см. ниже
...
- `src/ai/client.js` — Claude Sonnet 4.6, промпт поэта, grammar-first
- `src/queue.js` — серийная очередь генерации
- `src/access-codes.js` — 20 кодов доступа
- `src/index.js` — startup (таймер убран)
