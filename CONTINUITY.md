...
## Now
- **Шаг 1 «идеальный поэт» готов локально, не задеплоен.** Node-часть 15/15 тестов GREEN, Python-сайдкар 34/34 pytest GREEN, `npm run check` чист. Файлы не запушены, сервер не тронут. План зафиксирован в `C:\Users\user\.claude\plans\glistening-knitting-brooks.md`. Ждём решения пользователя задеплоить сейчас или позже.

## Next
- **Деплой Шага 1:** git add+commit+push → SSH на сервер → `mkdir -p ~/projects/rhyme-sidecar && rsync services/rhyme-sidecar/` → `python3.11 -m venv venv && pip install -r requirements.txt` → прогрев `ruaccent` (~150MB с HuggingFace) → копия `rhyme-sidecar.service` в /etc/systemd → `systemctl enable --now rhyme-sidecar` → смоук `curl /detect` на Зевс v2 (ожидаем fake=0 или ровно помеченные) и на жалобный Герыч (ожидаем fake≥1) → restart `podari-bot` → наблюдение 10-20 живых заказов.
- **После деплоя**: мониторить в journalctl `[ai] metrics: ... rhymes: {fake: N}` счётчики, `[pipeline] fake rhymes: ... — forcing critic+rewriter` триггеры, `[rhymes] sidecar unavailable` (не должно быть при работающем сервисе).
- **Решение о Шаге 2 (Best-of-N)** — через 1-2 недели после деплоя Шага 1, когда накопится distribution fake-rhyme rates на реальных заказах.
