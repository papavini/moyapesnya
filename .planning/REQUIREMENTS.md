...
- [ ] **METRICS-01**: Детектор банальных рифм на JS — список ≥28 кластеров (любовь/кровь, розы/слёзы/морозы, ночь/прочь/дочь, ты/мечты/цветы и др.), результат передаётся в критика
- [ ] **METRICS-02**: Счётчик слогов на JS (regex `[аеёиоуыэюя]`) — проверяет каждую строку припева на ≤12 слогов, передаёт нарушения в критика
...
- [ ] **METRICS-04**: Лексическое разнообразие — подсчёт уникальных слов / общее кол-во слов (MATTR-approx на JS), порог >0.60 для 200-словного текста
