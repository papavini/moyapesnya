...
| **rhyme-sidecar** | systemd (Python FastAPI) | 3100 | phonetic rhyme detector — [server-configs/rhyme-sidecar/](server-configs/rhyme-sidecar/) |
| **rdp-chromium-xvfb** | systemd + Xvfb :99 | 9223 (CDP) | autonomous chromium for SUNO refresh — [server-configs/rdp-chromium-xvfb/](server-configs/rdp-chromium-xvfb/) |

> **Docker НЕ используется** — все сервисы нативные через systemd. Docker ломал cookies из-за `$` в env_file.
>
> **Cookie/passkey refresh on-demand:** бот обновляет SUNO auth tokens через CDP :9223 только при 500/422 ошибках. Proactive timers отключены (создавали мусор). См. `docs/architecture.md`.
